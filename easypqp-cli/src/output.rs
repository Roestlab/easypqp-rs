...
// Parquet writer: feature-gated. By default this is a no-op stub so that the
// crate doesn't require a Parquet/Arrow dependency unless the feature is
// enabled. Enable the `parquet` feature and implement the writer with your
// preferred parquet/arrow crate (arrow2+parquet2 or apache-arrow's parquet).
#[cfg(not(feature = "parquet"))]
pub fn write_assays_to_parquet<P: AsRef<Path>>(
    _assays: &[PeptideProperties],
    _peptides: &[Peptide],
    _path: P,
    _insilico_settings: &InsilicoPQPSettings,
) -> Result<()> {
    // Return a helpful error explaining how to enable parquet support.
    Err(anyhow::anyhow!(
        "Parquet support is not enabled. Rebuild with the 'parquet' feature and enable the arrow/parquet optional dependencies in Cargo.toml to use this method."
    ))
}
#[cfg(feature = "parquet")]
pub fn write_assays_to_parquet<P: AsRef<Path>>(
    ...
    // Implement a simple mapping from assays -> row-oriented columns and
    // write as a single RecordBatch to a Parquet file.
    let assays = _assays;
    let peptides = _peptides;
    ...
    // Decide RT scale same as TSV writer: output-scaled by 100
    let rt_scale = 100.0_f32;

    for assay in assays {
        let peptide_idx = assay.peptide_index as usize;
        let decoy_flag = peptides[peptide_idx].decoy as i32;
        let modified_peptide = peptides[peptide_idx].to_string();
        let naked_peptide = remove_mass_shift(&modified_peptide);
        let protein = &peptides[peptide_idx].proteins;
        ...
        if non_zero_indices.len() < _insilico_settings.min_transitions as usize {
            continue;
        }

        let product_indices = if _insilico_settings.max_transitions > 0 {
            let mut sorted = non_zero_indices.clone();
            sorted.sort_by(|&a, &b| {
                assay.product.intensity[b]
                    .partial_cmp(&assay.product.intensity[a])
            ...
            sorted.into_iter()
                .take(_insilico_settings.max_transitions as usize)
                .collect()
        } else {
            non_zero_indices
        };
        ...
        for &i in &product_indices {
            ...
            v_transition_group_id.push(assay.peptide_index as i64);
            v_transition_id.push((assay.peptide_index as i64) * 1_000 + i as i64);
    ...
    // Helper to convert Vec<Option<String>> -> StringArray
    ...
    let arr_transition_group_id: ArrayRef = Arc::new(Int32Array::from(v_transition_group_id.iter().map(|&x| x as i32).collect::<Vec<_>>()));
    let arr_transition_id: ArrayRef = Arc::new(Int32Array::from(v_transition_id.iter().map(|&x| x as i32).collect::<Vec<_>>()));
...
/// Generate an HTML report directly from in-memory assays/peptides instead of a TSV file.
pub fn generate_html_report_from_memory(
    assays: &[PeptideProperties],
    peptides: &[Peptide],
    insilico_settings: &InsilicoPQPSettings,
    output_html: &str,
) -> Result<()> {
    // Build headers to match the TSV writer
    let headers = vec![
        "PrecursorMz".to_string(),
        "ProductMz".to_string(),
        "PrecursorCharge".to_string(),
        "ProductCharge".to_string(),
        "LibraryIntensity".to_string(),
        "NormalizedRetentionTime".to_string(),
        "PeptideSequence".to_string(),
        "ModifiedPeptideSequence".to_string(),
        "PeptideGroupLabel".to_string(),
        "LabelType".to_string(),
        "CompoundName".to_string(),
        "SumFormula".to_string(),
        "SMILES".to_string(),
        "Adducts".to_string(),
        "ProteinId".to_string(),
        "UniprotId".to_string(),
        "GeneName".to_string(),
        "FragmentType".to_string(),
        "FragmentSeriesNumber".to_string(),
        "Annotation".to_string(),
        "CollisionEnergy".to_string(),
        "PrecursorIonMobility".to_string(),
        "TransitionGroupId".to_string(),
        "TransitionId".to_string(),
        "Decoy".to_string(),
        "DetectingTransition".to_string(),
        "IdentifyingTransition".to_string(),
        "QuantifyingTransition".to_string(),
        "Peptidoforms".to_string(),
    ];

    // Build all_rows from assays/peptides following the same column order as the TSV writer
    let mut all_rows: Vec<Vec<String>> = Vec::new();

    // Use same RT scaling heuristic as TSV writer (explicit 100x output scale here)
    let rt_scale = 100.0_f32;
    ...
    for assay in assays {
        ...
        let protein_id_field = if protein_accessions.is_empty() { "".to_string() } else { protein_accessions.join(";") };
        let uniprot_id_field = protein_id_field.clone();
        let gene_name_field = if protein_genes.is_empty() { "".to_string() } else { protein_genes.join(";") };
        ...
        for &i in &product_indices {
            ...
            let row = vec![
                format!("{}", assay.precursor.precursor_mz),
                format!("{:.4}", assay.product.product_mz[i]),
                format!("{}", assay.precursor.charge),
                format!("{}", product_charge),
                format!("{}", assay.product.intensity[i] * 10_000.0),
                format!("{}", assay.retention_time * rt_scale),
                naked_peptide.clone(),
                modified_peptide.clone(),
                "".to_string(), // PeptideGroupLabel
                "".to_string(), // LabelType
                "".to_string(), // CompoundName
                "".to_string(), // SumFormula
                "".to_string(), // SMILES
                "".to_string(), // Adducts
                protein_id_field.clone(),
                uniprot_id_field.clone(),
                gene_name_field.clone(),
                fragment_type.to_string(),
                format!("{}", series_number),
                annotation,
                "".to_string(), // CollisionEnergy
                format!("{}", assay.precursor.ion_mobility),
                format!("{}", assay.peptide_index),
                format!("{}", assay.peptide_index * 1000 + i as u32),
                format!("{}", decoy as i8),
                "1".to_string(), // DetectingTransition
                "0".to_string(), // IdentifyingTransition
                "1".to_string(), // QuantifyingTransition
                "".to_string(), // Peptidoforms
            ];
    ...
    // The rest of the report generation mirrors the TSV-based function below.
    ...
    // Build Plotly plots (same logic as file-based reporter)
